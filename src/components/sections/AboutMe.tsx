...
import { easeOutExpo, staggerContainer, fadeSlideUp, scaleBlurIn } from "@/lib/animations";
...
export default function About() {
  ...
  return (
    ...
    >
      <div
        className="relative w-full bg-[#eeeeee] rounded-t-[80px] md:rounded-t-[120px] border-t-[3px] border-black/10 shadow-[0_-50px_80px_rgba(0,0,0,0.4)] -mt-24"
      >
        <div className="absolute top-20 left-0 w-full overflow-hidden opacity-[0.04] pointer-events-none select-none">
          <motion.h2
            style={{ x: xWatermark }}
            className="font-monument text-[20vw] whitespace-nowrap text-black uppercase will-change-transform"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            >
              <motion.div variants={fadeSlideUp}>
                <span className="text-red-600 font-mono text-xs tracking-[0.5em] uppercase mb-4 block">
                  [ 01 . Profile ]
                </span>
                <h2 className="font-monument text-5xl md:text-7xl text-[#111] uppercase leading-none">
                  About
                  <br />
              ...
              <motion.div
                ...
                className="space-y-6 pt-10 border-t border-black/10"
              >
              ...
              <motion.div variants={fadeSlideUp}>
                <Magnetic strength={0.15}>
                  <motion.a
                    href="https://docs.google.com/document/d/1nmQ92ORFSLzhF2T-D9lKVtEqzOEjVqY1/edit"
                    target="_blank"
                    rel="noopener noreferrer"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="group relative w-fit px-6 py-4 bg-[#111] text-white overflow-hidden rounded-xl flex items-center gap-3 shadow-lg cursor-pointer"
                  >
              ...
              >
                <SkillCard
                  ...
                  icons={[
                    ...
                    { icon: <FaReact />, color: "hover:text-[#61DAFB]" },
                    { icon: <SiNextdotjs />, color: "hover:text-[#000000]" },
                    { icon: <SiCplusplus />, color: "hover:text-[#00599C]" },
                    { icon: <SiArduino />, color: "hover:text-[#00979D]" },
                    { icon: <FaRobot />, color: "hover:text-[#00E5FF]" },
                    { icon: <FaMicrochip />, color: "hover:text-[#E34F26]" },
                  ]}
                />
        ...
        <div className="-mx-20 w-[calc(100%+10rem)] py-12 bg-black flex overflow-hidden -rotate-[2deg] translate-y-7">
          <motion.div
            animate={{ x: [0, -1000] }}
          ...
          >
            {[...Array(10)].map((_, i) => (
              <div
                key={i}
                className="flex gap-12 items-center text-white/20 font-monument text-2xl uppercase tracking-widest"
              >
...
function InfoItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="group border-l-2 border-black/5 pl-4 hover:border-red-600 transition-all duration-500">
      <p className="text-[9px] font-mono text-black/30 uppercase tracking-[0.2em]">
        {label}
      </p>
      <p className="font-monument text-sm text-[#111] uppercase mt-1 group-hover:translate-x-1.5 transition-transform duration-300">
        {value}
      </p>
    </div>
  );
...
function SkillCard({
  ...
  return (
    ...
    >
      <h4 className="font-monument text-[10px] text-black/40 uppercase tracking-[0.2em] mb-8 group-hover:text-red-600 transition-colors duration-300">
        {title}
      </h4>
      <div className="flex flex-wrap gap-5 items-center mt-auto">
        {icons.map((item, idx) => (
          <motion.div
            ...
            className={`text-3xl text-black/20 transition-colors duration-300 ${item.color} cursor-pointer`}
          >
            {item.icon}
          </motion.div>
        ))}
