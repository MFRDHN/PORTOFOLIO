...
const cardVariants = {
  hidden: { opacity: 0, y: 60, scale: 0.93, willChange: "transform, opacity" },
  visible: {
    opacity: 1,
    y: 0,
    scale: 1,
      ...
      <div className="relative z-10 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
        {projectsData.slice(0, 4).map((project, index) => (
          ...
          >
            ...
            <motion.div className="absolute -bottom-3 left-6 right-6 h-[2px] bg-red-600/80 rounded-full scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left" />
          </motion.div>
        ))}
      </div>
